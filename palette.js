...
// matugen's own roles (from the helper) -> the same shape buildPalette returns.
export function fromRoles(roles, dark) {
  const c = { ...roles };
  c.accent = chroma(c.primary) < 20 ? c.on_surface_variant : c.primary;
  c.seed = (roles.source_color || roles.primary).toLowerCase();
  c.dark = dark;
  return c;
}

// Seed colour -> the same named roles the matugen template produces.
