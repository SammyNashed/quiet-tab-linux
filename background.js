// Holds the one connection to the Linux helper, turns whatever the colour
// source is into a palette, and stores it for every New Tab page to pick up.
// The helper sends two things: a thumbnail of the wallpaper, and matugen's own
// colours (when matugen is set up), so the page can match the desktop exactly.
// An open native port keeps this service worker alive, so wallpaper changes
// land even when no New Tab is open.

import { buildPalette, extractSeeds, fromRoles } from './palette.js';
...
async function onHostMessage(msg) {
  ...
  if (msg.type === 'matugen') {
    await chrome.storage.local.set({ matugen: { stamp: msg.stamp, dark: msg.dark, light: msg.light } });
    await recompute();
    return;
  }
...
async function recompute() {
  ...
  const { wallpaper, matugen } = await chrome.storage.local.get(['wallpaper', 'matugen']);
  if (settings.source === 'matugen' && matugen && matugen.dark && matugen.light) {
    await chrome.storage.local.set({ palette: {
      seed: matugen.dark.source_color || matugen.dark.primary,
      dark: fromRoles(matugen.dark, true),
      light: fromRoles(matugen.light, false),
    } });
    return;
  }
  // No matugen colours (yet): build from the wallpaper instead.
  const seed = pickSeed({ ...settings, source: settings.source === 'matugen' ? 'wallpaper' : settings.source }, wallpaper);
  const palette = {
    seed,
